...
| `CreateDrawConfigurationDto` | `code`, `time`, `tuesdayOnly?`, `autoGenerateShifts?`, `singleDate?`, `lockSecondsBefore?`, `reopenSecondsAfter?`, `active?` | Código slug; hora `HH:mm[:ss]`; si `autoGenerateShifts=false`, `singleDate` es obligatorio. |
| `SoftDeleteDrawConfigurationDto` | `reason?` | Desactiva la configuración sin borrar historial. |
| `HardDeleteDrawConfigurationDto` | `adminPassword`, `confirmation`, `reason?` | `confirmation` debe ser `DELETE_DRAW_CONFIGURATION`; reautentica al admin antes de borrar en cascada. |
| `AutoGenerateDrawShiftsDto` | `date?` | Fecha `YYYY-MM-DD`; si se omite usa el día actual del servidor. |
...
| `sorteos_config` | Configuración recurrente o de fecha única: código, hora, ventanas, autogeneración y baja lógica. |
...
| POST | `/draws/configurations` | `sorteos.create` | Crea código, hora, modo de autogeneración y ventanas operativas. |
...
| GET | `/draws/configurations/:configurationId/delete-impact` | `sorteos.delete` | Calcula cuántos turnos, ventas, resultados, pagos, límites y bloqueos serían afectados por un borrado físico. |
| PATCH | `/draws/configurations/:configurationId/soft-delete` | `sorteos.delete` | Baja lógica: marca inactivo, guarda motivo y evita uso futuro. |
| DELETE | `/draws/configurations/:configurationId` | ADMIN, `sorteos.delete` | Borrado físico con contraseña admin y confirmación literal; elimina dependencias en cascada controlada. |
| GET | `/draws/shifts/active` | `turnos.read` | Lista turnos actualmente operables; si recibe `date`, autogenera los faltantes para ese día antes de listar. |
| GET | `/draws/shifts` | `turnos.read` | Lista turnos por fecha/estado con paginación; si recibe `date`, autogenera los faltantes para ese día antes de listar. |
| POST | `/draws/shifts` | `turnos.create` | Abre un turno para configuración y fecha. |
| POST | `/draws/shifts/auto-generate` | `turnos.create` | Genera turnos faltantes de todas las configuraciones activas aplicables al día. |
...
- `autoGenerateShifts=true`: el sorteo queda recurrente; la API puede crear el
  turno de cada día cuando el administrador lo solicite o cuando se consulte el
  listado de turnos para una fecha.
- `autoGenerateShifts=false` + `singleDate=YYYY-MM-DD`: el sorteo solo puede
  operar ese día específico. `POST /draws/shifts` rechaza fechas distintas.
- `tuesdayOnly=true` sigue funcionando como restricción adicional: aunque sea
  recurrente, solo se autogenera si la fecha cae martes.

El borrado lógico es la opción normal para errores operativos: conserva ventas,
turnos y reportes históricos. El borrado físico existe para limpieza
administrativa excepcional; antes debe consultarse `delete-impact` y enviar la
contraseña del administrador autenticado. La API reautentica contra Supabase y
solo procede si la sesión devuelta pertenece al mismo `auth_user_id` del JWT.
...
- `ADMIN` puede ver la empresa completa o filtrar por `sellerId`.
- `VENDEDOR` siempre queda forzado a su propio perfil vendedor, aunque envíe
  otro `sellerId` en la query.
- Un usuario no administrador sin perfil vendedor recibe `403` porque la API no
  puede construir estadísticas propias sin asignación segura.
...
| DELETE | `/notifications/:notificationId` | `notificaciones.delete` | Elimina una notificación propia de la bandeja; otra cuenta obtiene `404`. |
...
Para una UX donde "Leer todas" limpie el listado, el cliente debe consultar
`GET /notifications?unread=true`: después de `PATCH /notifications/read-all`,
esa query queda vacía. Para lectura individual con modal, el payload real viene
en `data.payload`; al cerrar el modal se puede refrescar `unread=true` o llamar
`DELETE /notifications/:notificationId` si la intención es retirarla
definitivamente de la bandeja.
